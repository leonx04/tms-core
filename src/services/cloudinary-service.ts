...
export const getCloudinaryConfig = async (configId: string): Promise<ProjectCloudinaryConfig | null> => {
    try {
...
export const getCloudinaryConfigByProjectId = async (projectId: string): Promise<ProjectCloudinaryConfig | null> => {
    try {
...
export const deleteCloudinaryConfig = async (configId: string): Promise<boolean> => {
    try {
        const configRef = ref(database, `projectCloudinary/${configId}`)
        const snapshot = await get(configRef)
...
export const storeCloudinaryUpload = async (
    ...
    try {
        ...
        const uploadId = uploadRef.key as string
        ...
        const uploadData = {
            ...
            commentId: uploadResult.commentId,
            createdAt: timestamp,
        }

        await set(uploadRef, uploadData)
        return uploadId
    } catch (error) {
...
export const getProjectCloudinaryUploads = async (projectId: string): Promise<any[]> => {
    try {
...
export const getTaskCloudinaryUploads = async (projectId: string, taskId: string): Promise<any[]> => {
    try {
        const uploadsRef = ref(database, `projectCloudinaryUploads/${projectId}`)
        const snapshot = await get(uploadsRef)
...
export const getCommentCloudinaryUploads = async (projectId: string, commentId: string): Promise<any[]> => {
    try {
        const uploadsRef = ref(database, `projectCloudinaryUploads/${projectId}`)
        const snapshot = await get(uploadsRef)
