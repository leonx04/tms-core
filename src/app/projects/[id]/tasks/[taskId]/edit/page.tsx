...
import type { MediaAttachment, Project, Task, TaskHistory } from "@/types";
...
const isDifferent = (oldValue: any, newValue: any): boolean => {
  ...
  if (
    ...
    (oldValue instanceof Date ||
      (typeof oldValue === "string" && oldValue.match(/^\d{4}-\d{2}-\d{2}/)))
  ) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (
          taskData.mediaAttachments &&
          Array.isArray(taskData.mediaAttachments)
        ) {
        ...
        const canEditTask = userRoles.some((role) =>
    ...
    try {
      ...
      const mediaAttachments: MediaAttachment[] = [
        ...existingMedia.filter(
          (media) => !mediaToRemove.includes(media.publicId)
        ),
        ...uploadedMedia,
      ].map(
        (media) =>
          ...
          ) as MediaAttachment // Explicitly cast to MediaAttachment
      );
      ...
      const updates: Partial<Task> = {
        ...
        mediaAttachments, // Ensure type compatibility
      };
      ...
      if (isDifferent(originalTask.percentDone, percentDone)) {
        changes.push({
          field: "percentDone",
          oldValue: originalTask.percentDone,
          newValue: percentDone,
        });
      }
      ...
      if (isDifferent(originalTask.estimatedTime, estimatedTime)) {
        changes.push({
          field: "estimatedTime",
          oldValue: originalTask.estimatedTime,
      ...
      if (isDifferent(originalTask.parentTaskId, parentTaskId)) {
        changes.push({
          field: "parentTaskId",
          oldValue: originalTask.parentTaskId,
      ...
      if (isDifferent(originalMedia, mediaAttachments)) {
        changes.push({
          ...
          oldValue: originalMedia,
          newValue: mediaAttachments,
        });
      }
  ...
  return (
    <div className="bg-background min-h-screen">
      <main className="container mx-auto px-4 py-8">
        ...
        <PageHeader
          title="Edit Task"
          description={`Update task details for ${project.name}`}
        />
              ...
              <div className="space-y-4">
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value={TASK_STATUS.RESOLVED}>
                      Resolved
                    </SelectItem>
                                ...
                                >
                                  ...
                                  {mediaToRemove.includes(media.publicId) ? (
                                    <button
                                      type="button"
                                      onClick={() =>
                                        handleRestoreMedia(media.publicId)
                                      }
                                ...
                                {uploadedMedia.map((media, index) => (
                                  ...
                                  >
                                    ...
                                    <button
                                      type="button"
                                      onClick={() =>
                                        handleRemoveUploadedMedia(index)
                                      }
                                      className="text-xs text-destructive mt-1 hover:underline"
                                    >
                                      Remove
