...
export function MediaGallery({
  ...
  const isVideo = (item: any) => {
    return item.resourceType === "video" || (item.format && ["mp4", "webm", "ogv"].includes(item.format.toLowerCase()))
  }
          ...
          <CardContent className="p-0 relative">
            {isImage(item) ? (
              ...
              <div className="aspect-video relative bg-muted">
                <video src={item.url} controls={false} muted className="w-full h-full object-cover" />
              ...
              <div className="aspect-square flex items-center justify-center bg-muted">
                <div className="text-center p-4">
                  <FileImage className="h-8 w-8 mx-auto mb-2" />
                  <p className="text-xs truncate max-w-full">{item.format || "File"}</p>
                </div>
              </div>
            )}
